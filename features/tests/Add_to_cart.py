...
def search_for_mango(context):
    context.driver.find_element(By.ID, 'search').send_keys('Mango')
    context.driver.find_element(Search_Btn).click()
    sleep(3)

@then('Mango Search result is shown')
def search_for_mango_result(context):
    actual_text =context.driver.find_element(Search_Result_Header).text
    assert 'Mango' in actual_text
